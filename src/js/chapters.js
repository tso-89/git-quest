  ...
  var chapters = [
    ...
    // ------------------------------------------------------------------- 02
    {
      ...
      blocks: [
        ...
        { note: 'Each commit is a full snapshot of the folder, not a list of edits. That is why checking out an old commit is instant, and why nothing is ever half-restored.', kind: 'key' },
        { h: 'Make one' },
        { cmds: [
    ...
    // ------------------------------------------------------------------- 04
    {
      ...
      blocks: [
        ...
        { h: 'What push means' },
        { p: '<code>git push</code> sends any commits your local branch has that the remote does not. It never sends uncommitted work — if you have not committed it, pushing does nothing for it. That surprises everyone once.' },
        { cmds: [
    ...
    // ------------------------------------------------------------------- 05
    {
      ...
      blocks: [
        ...
        { note: 'Working directory, then <code>git add</code>, then staging area, then <code>git commit</code>, then history, then <code>git push</code>, then GitHub. Four arrows. That is the loop.', kind: 'key' },
      ...
      outro: 'You now know enough git to work alone, safely, forever. The next chapters are about other people and other machines.'
    },
    ...
    {
      id: 'branches',
      title: 'Branches, and undoing anything',
      subtitle: 'Cheap parallel universes, and the four panic buttons',
      xp: 180,
      pane: 'terminal',
      setup: function (eng) {
        var root = HOME + '/my-first-repo';
      ...
      blocks: [
        ...
        { h: 'The four panic buttons' },
        { p: 'Learn which one you need by what you are trying to undo. This table is worth keeping.' },
        { compare: {
          heads: ['I want to…', 'Use'],
          rows: [
      ...
      quest: {
        title: 'Branch, merge, undo, and find it again',
        brief: 'Do a piece of work on a branch, merge it, undo it safely, then prove nothing was lost.',
        steps: [
          {
    ...
    // ------------------------------------------------------------------- 08
    {
      ...
      subtitle: 'Pull, conflict, resolve, pull request',
      xp: 180,
      pane: 'terminal',
      setup: function (eng) {
        var root = HOME + '/my-first-repo';
        ...
        eng.repo.index = { 'README.md': '# my-first-repo\n\nLearning git properly, finally.\n\n## Status\n\nStill learning. Chapter eight of ten.' };
        eng.commit('Note progress through the lesson');
        eng.repo.remote = 'origin';
        eng.repo.reflog = [];
      },
      blocks: [
        { p: 'Sam has pushed a change to <code>README.md</code> while you were working on the same lines. Neither of you did anything wrong. Git will not guess which version is right, and you would not want it to.' },
        ...
        { h: 'Then: pull requests' },
        { p: 'On a team you rarely push straight to <code>main</code>. You push a branch and open a <strong>pull request</strong>: a page that shows your commits, the full diff, and a place for people to argue about it before it lands. It is a code review with a merge button attached.' },
        { p: 'Every agent-written change is worth this treatment, even when you work alone. A pull request is the one place you reliably read a diff in full.' },
        { cmds: [
          ...
          { cmd: 'echo "Shipped v1. Still learning, chapter eight of ten." >> README.md', desc: 'the version that should exist' },
    ...
    // ------------------------------------------------------------------- 09
    {
      ...
      blocks: [
        ...
        { h: 'Four habits, in order of how much they will save you' },
        { p: '<strong>1. Commit before you prompt.</strong> A clean working tree before the agent starts means everything it changes shows up in <code>git status</code> as its work, not tangled with yours. It also means <code>git restore .</code> is a complete undo.' },
        { p: '<strong>2. Read the diff, not the summary.</strong> The summary is written by the thing that made the change. It is usually accurate and occasionally omits the interesting part. <code>git diff</code> cannot omit anything.' },
        { p: '<strong>3. One branch per task.</strong> So "this went badly, delete it" is one command instead of an afternoon.' },
        { p: '<strong>4. Keep secrets structurally impossible.</strong> An agent that can read your files can read <code>.env</code>, and an agent that can commit can commit it. <code>.gitignore</code> is the guardrail, and it belongs in the repository so every agent gets the same one.' },
      ...
      quest: {
        ...
        brief: 'Commit your own work, run the agent, read the diff, and stop the thing it got wrong from being committed.',
        steps: [
          {
